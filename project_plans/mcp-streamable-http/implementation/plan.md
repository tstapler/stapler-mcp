...
| `shutdown_cleanup` | `async fn shutdown_cleanup(daemon: Rc<Daemon>, mut browser: Rc<NativeBrowser>)` factored out of the existing post-`daemon.run()` sequence (today inline at `main.rs:462-485`, unchanged in shape — just moved) | Called from exactly one place: after `run_daemon`'s single `tokio::join!` (accept loop + bridge consumer + optional HTTP server, each awaited with a bounded grace timeout) completes — regardless of whether shutdown was triggered by `SHUTDOWN_TOOL` or `SIGTERM`. Exactly one cleanup code path, not two |
...
##### Task 7.2.1d: Document the token-distribution warning (~5 min)
- State explicitly: never paste a literal token into the git-tracked `mcp-servers.json`; use `stapler-mcp --print-config` and paste the result into `~/.claude.json`'s user-scoped `mcpServers` entry — Claude Code's real, documented user-level MCP config file (`code.claude.com/docs/en/mcp.md`), which is a standalone file directly in `$HOME`, distinct from the `~/.claude/` subdirectory this repo's own `CLAUDE.md` documents as a dotfiles-managed git symlink. Include the verification snippet from `design/ux.md` surface 1 an operator should run first, exactly as written there (a naive `git -C <repo> ls-files --error-unmatch ~/.claude.json` is wrong — it raises `fatal: ... outside repository` rather than a clean "not tracked" result whenever the target isn't inside the repo's own working-tree directory, which is the common case; the corrected snippet resolves both paths with `readlink -f` first and branches on containment before ever calling `git ls-files`, verified against three real cases — outside the repo, inside-and-tracked, inside-and-untracked — by execution during `pm:triad-review`, 2026-09-14) before trusting `~/.claude.json` as untracked; if it's tracked, gitignore it there or use a `headersHelper` script outside any tracked directory instead. Note Claude Code's reported `${ENV_VAR}`-in-`headers` bug as the reason env-var indirection isn't a safe substitute either.
- Files: `README.md`
